    ...
    <a-layout>
      ...
      <a-layout>
        ...
        <a-layout-content :style="{ margin: '24px 16px', padding: '24px', background: '#fff', minHeight: '280px' }" style="overflow: hidden">
            <a-page-header title="工单详情" :sub-title="id" @back="() => router.push('ticket')">
                <template #tags>
                ...
                <template #extra>
                    <a-dropdown key="1">
                      <template #overlay>
                        <a-menu @click="updateStatus">
                          <a-menu-item key="-1">关单（谨慎操作）</a-menu-item>
                          <a-menu-item key="0">未处理</a-menu-item>
                          <a-menu-item key="1">处理中</a-menu-item>
                          <a-menu-item key="2">待回复</a-menu-item>
                        </a-menu>
                      </template>
                      <a-button>更改状态</a-button>
                    </a-dropdown>
                    <a-button @click="deleteTicket(data._id.$oid)" key="2" type="danger">删除工单</a-button>
                </template>
                <a-row type="flex">
                ...
                <div style="padding: 20px">
                    ...
                    <a-list :data-source="reply" :header="`共 ${reply.length} 条回复`" style="overflow: scroll; max-height: 250px;">
                        <a-list-item v-for="d in reply">
                            <a-comment :author="d.sender"><template #avatar><a-avatar><template #icon><UserOutlined/></template></a-avatar></template><template #content>{{ d.content }}</template><template #datetime>{{ d.time }}</template></a-comment>
                        </a-list-item>
                    </a-list>
...
import * as monaco from 'monaco-editor';
    ...
    const sendReply = () => {
        loading.value = true;
        Axios({
            url: '/api/admin/ticket/reply?id=' + vroute.params.ticket_id,
            method: 'POST',
    ...
    // 更新工单状态
    const updateStatus: MenuProps['onClick'] = e => {
      console.log('click', e);
      if (e['key'] == -1){
        Modal.confirm({
          title: '确认关闭工单？',
          content: '关闭后玩家和您都将无法进行回复，且玩家无法主动操作开单。是否继续？',
          onOk(){
            Axios({
              method: 'POST',
              url: '/api/admin/ticket/update',
              data: {
                ticket_id: vroute.params.ticket_id,
                status: -1
              }
            })
            .then(function(Response){
